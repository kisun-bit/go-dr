...
func SyncExecBin(cmdString string) (r int, out string, err error) {
	cs := strings.Fields(cmdString)
...
func SyncExecShell(cmd string) Status {

	c := NewCmd(OsCmdCaller())
	in := bytes.NewBuffer(nil)
	in.WriteString(fmt.Sprintf("%s\n", cmd))

	return <-c.StartWithStdin(in)
}

func SyncExecShellWithTimeout(cmd string, timeout time.Duration) Status {

	// Start a long-running process, capture stdout and stderr
	cmd_ := NewCmd(OsCmdCaller())

	in := bytes.NewBuffer(nil)
	cmdLine := fmt.Sprintf("%s\n", cmd)
	in.WriteString(cmdLine)
	statusChan := cmd_.StartWithStdin(in) // non-blocking
...
func (rw *OutputBuffer) Lines() []string {
	...
	for s.Scan() {
		if utf8Raw, err := datahandle.Gbk2Utf8(s.Bytes()); err != nil {
			log.DLogger.Fmt.Warnf("parse encoding failed: %v", err)
			rw.lines = append(rw.lines, s.Text())
		} else {
			rw.lines = append(rw.lines, string(utf8Raw))
		}
	}
	rw.Unlock()
	return rw.lines
